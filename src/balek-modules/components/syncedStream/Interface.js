define([
  ...
  return declare(
    "moduleBaseSyncedStreamInterface",
    [baseInterface, stateSyncer, remoteCommander],
    {
      ...
      beginStreaming: function (outputCallback = null) {
        ...
        if (
          ...
          this._instanceCommands.preloadChunks
        ) {
          ...
          if (this._streamLength > 0) {
            if (this._streamLength > this._preloadSize) {
              this._streamStartPosition =
                this._streamLength - this._preloadSize;
              this._streamPosition = this._streamStartPosition;
            }
          }

          this._instanceCommands
            .preloadChunks(this._preloadSize)
            .then(
              lang.hitch(this, function (firstLoad) {
                if (
                  firstLoad.streamSize &&
                  firstLoad.streamSize !== this._streamLength
                ) {
                  this._streamLength = firstLoad.streamSize;
                  if (firstLoad.streamSize > this._preloadSize) {
                    this._streamStartPosition =
                      firstLoad.streamSize - this._preloadSize;
                    this._streamPosition = this._streamStartPosition;
                  }
                }
                ...
                if (firstLoad.preloadChunks) {
                  for (const chunkPosition in firstLoad.preloadChunks) {
                    this._chunks[chunkPosition] =
                      firstLoad.preloadChunks[chunkPosition];
                  }
                }
      ...
      continueStreaming: function () {
        if (this.outputCallback !== null) {
          if (
            !(
              this._streamPosition > this._streamLength ||
              this._chunks[this._streamPosition] === undefined
            )
          ) {
