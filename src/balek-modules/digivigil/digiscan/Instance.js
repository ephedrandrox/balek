  ...
  return declare("moduleDigivigilDigiscanInstance", _SyncedCommanderInstance, {
    ...
    constructor: function (args) {
      ...
      this._commands = {
        ...
        retrieveCaptureImagePreview: lang.hitch(
          this,
          this.retrieveCaptureImagePreview
        ),

        retrieveCaptureImageCheckHash: lang.hitch(
          this,
          this.retrieveCaptureImageCheckHash
        ),
        retrieveCaptureCheckHash: lang.hitch(
          this,
          this.retrieveCaptureCheckHash
        ),
      ...
      this._interfaceState.set(
        ...
        this.availableCaptures._componentKey
      );
      ...
      this._interfaceState.set(
        ...
        this.captureSets._componentKey
      );
      ...
      if (userKey != null) {
        ...
        this.userCaptureSets =
          this._moduleController.getCaptureSetsForUser(userKey);
        this.captureSets.relayState(this.userCaptureSets);
      }

      this.uiState = new SyncedMapInstance({
        _instanceKey: this._instanceKey,
        _commonName: "UIState",
      });
      ...
      this.settingsInstance = new SettingsInstance({
        _instanceKey: this._instanceKey,
        _sessionKey: this._sessionKey,
        _userKey: this._userKey,
        _Controller: this,
      ...
      //Set Main Instance keys for interface
      this._interfaceState.set("settingsInstanceKeys", {
        instanceKey: this.mainInstance._instanceKey,
      ...
      let session = this.sessionsControllerCommands.getSessionByKey(
        this._sessionKey
      );
        ...
        lang.hitch(this, function (name, oldState, newState) {
          if (
            ...
            if (sessionState.get("name").toString() === "Main Web Session") {
              ...
              this.sessionsControllerCommands
                .unloadSession(this._sessionKey)
                .catch(
    ...
    setShowHelpfulHints: function (show, resultCallback) {
      if (typeof show === "boolean") {
        ...
        resultCallback({
          ERROR: "instance -> setShowHelpfulHints: activeView is not a string",
        });
      }
    },
    setUIActiveView: function (activeView, resultCallback) {
      if (typeof activeView === "string") {
        ...
        resultCallback({
          ERROR: "instance -> setUIActiveView: activeView is not a string",
        });
      }
    },
    ...
    updateCaptureImage: function (updateEntry, resultCallback) {
      this._moduleController
        .updateCaptureImage(updateEntry)
        .then(
          lang.hitch(this, function (Result) {
            resultCallback({ SUCCESS: Result });
          })
    ...
    retrieveCaptureImageCheckHash: function (captureID, resultCallback) {
      this._moduleController
        .retrieveCaptureImageCheckHash(captureID)
        .then(
          lang.hitch(this, function (Result) {
            resultCallback({ SUCCESS: Result });
          })
    ...
    retrieveCaptureCheckHash: function (captureID, resultCallback) {
      this._moduleController
        .retrieveCaptureCheckHash(captureID)
        .then(
          lang.hitch(this, function (Result) {
            resultCallback({ SUCCESS: Result });
          })
    ...
    retrieveCaptureImage: function (captureID, resultCallback) {
      this._moduleController
        .retrieveCaptureImage(captureID)
        .then(
          lang.hitch(this, function (Result) {
            resultCallback({ SUCCESS: Result });
          })
    ...
    retrieveCaptureImagePreview: function (captureID, resultCallback) {
      this._moduleController
        .retrieveCaptureImagePreview(captureID)
        .then(
          lang.hitch(this, function (Result) {
            resultCallback({ SUCCESS: Result });
          })
    ...
    deleteCaptureSet: function (id, resultCallback) {
      if (typeof id === "string" && typeof resultCallback === "function") {
        ...
        resultCallback({
          ERROR: "instance -> deleteCaptureSet: id is not a string",
        });
      }
    },
    ...
    clearCaptureSet: function (id, resultCallback) {
      if (typeof id === "string" && typeof resultCallback === "function") {
        ...
        resultCallback({
          ERROR: "instance -> clearCaptureSet: id is not a string",
          is: id,
        });
      }
    },
    selectCaptureSet: function (id, resultCallback) {
      if (typeof id === "string" && typeof resultCallback === "function") {
        ...
        resultCallback({
          ERROR: "instance -> selectCaptureSet: id is not a string",
        });
      }
    },
    selectCapture: function (id, resultCallback) {
      if (typeof id === "string" && typeof resultCallback === "function") {
        ...
        resultCallback({
          ERROR: "instance -> selectCapture: id is not a string",
        });
      }
    },
    clearSelectedCaptures: function (resultCallback) {
      if (typeof resultCallback === "function") {
        ...
        resultCallback({
          ERROR: "instance -> clearSelectedCaptures: not expected",
        });
      }
    },
    removeCaptureFromSet: function (captureSetID, captureID, resultCallback) {
      if (
        typeof captureSetID === "string" &&
        typeof captureID === "string" &&
        typeof resultCallback === "function"
      ) {
        this._moduleController
          .removeCaptureFromSet(captureSetID, captureID)
          .then(
            lang.hitch(this, function (Result) {
              resultCallback({ SUCCESS: Result });
            })
    ...
    addCaptureToSet: function (captureSetID, captureID, resultCallback) {
      if (
        ...
        this._moduleController
          .addCaptureToSet(captureSetID, captureID)
          .then(
            lang.hitch(this, function (Result) {
              resultCallback({ SUCCESS: Result });
            })
        ...
        resultCallback({
          ERROR: "instance -> addCaptureToSet: unexpected Data",
        });
      }
    },
    addCaptureSet: function (CaptureSet, resultCallback) {
      let userKey = this.sessionsControllerCommands
        .getSessionByKey(this._sessionKey)
        .getUserKey();
      this._moduleController
        .addCaptureSet(CaptureSet, userKey)
        .then(
          lang.hitch(this, function (Result) {
            resultCallback({ SUCCESS: Result });
          })
    ...
    getCaptureSetSyncedMap: function (captureSetID, resultCallback) {
      ...
      this._moduleController
        .getCaptureSetSyncedMap(captureSetID, this._instanceKey)
        .then(
          lang.hitch(this, function (Result) {
    ...
    getCaptureSyncedMap: function (captureID, resultCallback) {
      ...
      this._moduleController
        .getCaptureSyncedMap(captureID, this._instanceKey)
        .then(
          lang.hitch(this, function (Result) {
            resultCallback(Result);
          })
    ...
    createCaptureSet: function (name, appendAll, resultCallback) {
      if (typeof name === "string" && typeof appendAll === "boolean") {
        ...
        this.addCaptureSet(
          { name: name, appendAll: appendAll, captures: captures },
          resultCallback
        );
        resultCallback({
          SUCCESS: "New Capture Set " + name + "added successfully",
        });
      } else {
        resultCallback({
          ERROR: "instance -> newAllSet: setName is not a string",
        });
      }
    },
    createInitialCaptureSetCaptures: function (appendAll) {
      ...
      if (appendAll === true) {
        for (const captureID in this.userCaptures) {
          if (
            this.userCaptures.hasOwnProperty(captureID) &&
            captureID !== "_watchCallbacks"
          ) {
