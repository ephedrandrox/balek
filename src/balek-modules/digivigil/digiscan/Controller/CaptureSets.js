  ...
  return declare("digivigilScapturaCaptureSetsController", null, {
    ...
    constructor: function (args) {
      ...
      if (this._module === null) {
        ...
        this._captureSetsDatabase = new captureSetsDatabase({
            ...
            .then(
              lang.hitch(this, function (CaptureSets) {
                if (Array.isArray(CaptureSets)) {
                  ...
                  Resolve({
                    SUCCESS: "getCaptureSets database results Loaded",
                  });
                } else {
                  Reject({
                    Error: "CaptureSets are not an array! loadCaptureSets",
                  });
                }
              })
            )
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          ...
          if (CaptureSet && typeof userKey === "string") {
            console.log(
              "Controller Adding Capture Set to Database",
              CaptureSet
            );
            this._captureSetsDatabase
              ...
              .then(
                lang.hitch(this, function (Result) {
                  ...
                  try {
                    ...
                    this.get(id)
                      .then(
                        lang.hitch(this, function (DBCaptureSet) {
    ...
    updateToUserList: function (CaptureSet) {
      if (
        ...
        CaptureSet.userKey &&
        CaptureSet._id &&
        typeof CaptureSet._id.toString === "function"
      ) {
    ...
    updateStatefulCaptureSet: function (CaptureSet) {
      if (
        CaptureSet &&
        CaptureSet.CaptureSet &&
        CaptureSet.CaptureSet.name &&
        typeof CaptureSet.CaptureSet.appendAll === "boolean" &&
        typeof CaptureSet.CaptureSet.captures === "object"
      ) {
        let statefulCaptureSet = this.getStatefulCaptureSet(
          CaptureSet._id.toString()
        );
        //statefulCaptureSet.set("CaptureSet", CaptureSet.CaptureSet);

        let capturesArray = Object.keys(CaptureSet.CaptureSet.captures).filter(
        ...
        statefulCaptureSet.set("filterSettings", {
          appendAll: CaptureSet.CaptureSet.appendAll,
        });
      }
    },
    getStatefulCaptureSet: function (id) {
      if (!this.statefulCaptureSetsByCaptureID[id]) {
        this.statefulCaptureSetsByCaptureID[id] =
          new this.StatefulCaptureSetChecksumsSet({});
      }
    ...
    get: function (id) {
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          this._captureSetsDatabase.getCaptureSet(id).then(
            lang.hitch(this, function (Result) {
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          ...
          if (statefulCaptureSet && captureSet && captureSet.CaptureSet) {
            ...
            this._captureSetsDatabase
              .updateCaptureSet(captureSetId, CaptureSet)
              .then(
                lang.hitch(this, function (Result) {
                  // console.log("Capture Set Updated", Result);
                  // try{
                  ...
                  if (Result.modifiedCount === 1) {
                    this.captureSets.set(captureSetId, {
                      _id: captureSetId,
                      CaptureSet: CaptureSet,
                    });
                  } else {
                    console.log(
                      "Database didnt save captures Set:",
                      CaptureSet,
                      captureSetId
                    );
                  }
    ...
    removeCaptureFromSets: function (captureID) {
      ...
      this._captureSetsDatabase.getCaptureSets().then(
        lang.hitch(this, function (CaptureSets) {
          if (Array.isArray(CaptureSets)) {
            CaptureSets.forEach(
              lang.hitch(this, function (CaptureSet) {
                let id = CaptureSet._id.toString();
                if (
                  CaptureSet.CaptureSet.captures[captureID] &&
                  CaptureSet.CaptureSet.captures[captureID].inSet === true
                ) {
                ...
                lang.hitch(this, function (Result) {
                  ...
                  if (Result.modifiedCount === 1) {
                    ...
                    console.log("Updating user list", {
                      _id: captureSetId,
                      userKey: captureSet.userKey,
                      CaptureSet: CaptureSet,
                    });

                    this.updateToUserList({
                      _id: captureSetId,
                      userKey: captureSet.userKey,
                      CaptureSet: CaptureSet,
                    });
              ...
              .then(
                lang.hitch(this, function (Result) {
                  ...
                  if (Result.modifiedCount === 1) {
                    ...
                    let capturesArray = Object.keys(statefulCaptureSet).filter(
                      (key) =>
                        !(
    ...
    checkAndReturnValidCaptureSet(CaptureSet) {
      let validCaptureSet = {};
      if (
        typeof CaptureSet.name === "string" &&
        typeof CaptureSet.appendAll === "boolean" &&
