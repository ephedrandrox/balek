define([
  ...
  return declare("digivigilDigiscanCapturesController", null, {
    ...
    constructor: function (args) {
      ...
      let connectControllerInstanceCommands =
        new ConnectControllerInstanceCommands();
      ...
      if (this._module === null) {
        ...
        connectControllerInstanceCommands
          .getCommands()
          .then(
            lang.hitch(this, function (connectCommands) {
              this.connectControllerCommands = connectCommands;

              this._capturesDatabase = new capturesDatabase({
                _instanceKey: this._instanceKey,
              });

              this._capturesImagesDatabase = new capturesImagesDatabase({
                _instanceKey: this._instanceKey,
              });
          ...
          .catch(
            lang.hitch(this, function (error) {
              console.log(
                "digivigilDigiscanCapturesController connectControllerInstanceCommands.getCommands() Error:",
                error
              );
            })
          );
    ...
    resetCaptureMemory: function () {
      ...
      let capturesByUserKeyArray = Object.keys(capturesByUserKey).filter(
        (key) =>
          !(key.includes("_watchCallbacks") || key.includes("filterSettings"))
      );
      console.log("Clearing these:", capturesByUserKeyArray);
      capturesByUserKeyArray.forEach(
        lang.hitch(this, function (userKey) {
          let captures = capturesByUserKey[userKey];
          console.log("Clearing these:", userKey, captures);

          let capturesArray = Object.keys(captures).filter(
            (key) =>
              !(
    ...
    load: function () {
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          this._capturesDatabase
            .getCaptures()
            .then(
              lang.hitch(this, function (Captures) {
                if (Array.isArray(Captures)) {
                  Captures.forEach(
                    lang.hitch(this, function (Capture) {
                      let id = Capture._id.toString();
                      this.checkAndReturnValidCapture(Capture.capture)
                        .then(
                          lang.hitch(this, function (VerifiedCapture) {
                        ...
                        .catch(
                          lang.hitch(this, function (Error) {
                            console.log(
                              "Error Verifying Capture",
                              Error,
                              Capture,
                              Capture.capture.signature.ownerUserKey,
                              Capture.capture.signature["Public Key"]
                            );
                            //remove from database
                            this._capturesDatabase
                              .removeCapture(Capture._id)
                              .then(
                                lang.hitch(this, function (Result) {
    ...
    loadCaptureImageInfo: function (CaptureID, CaptureObjectID = null) {
      ...
      this._capturesImagesDatabase
        .getCaptureImageInfo(CaptureID)
        .then(
          lang.hitch(this, function (CaptureImageInfo) {
            if (
              CaptureImageInfo &&
              CaptureImageInfo.CaptureImage &&
              CaptureImageInfo.CaptureImage.signature &&
              CaptureImageInfo.CaptureImage.image &&
              CaptureImageInfo.CaptureImage.image.data
            ) {
              this.checkAndReturnValidCaptureImage(
                CaptureImageInfo.CaptureImage
              )
                .then(
                  lang.hitch(this, function (validCaptureImage) {
                    this.addImageInfoToCaptureStateful(
                      validCaptureImage.signature,
                      CaptureObjectID
                    );
                  })
                )
    ...
    addCaptureToCaptureSets: function (id, Capture) {
      ...
      if (
        ...
        let userCaptureSets = this._instanceController.getCaptureSetsForUser(
          Capture.capture.signature.ownerUserKey
        );
        //filter out _watchCallbacks and create an array of Capture Set IDs
        let captureSetsArray = Object.keys(userCaptureSets).filter(
          (key) => !key.includes("_watchCallbacks")
        );
        //iterate through the array of Capture Set IDs
        captureSetsArray.forEach(
          lang.hitch(this, function (captureSetID) {
            //get statefulCaptureSet which has FilterSettings and CaptureIDs
            let captureSet =
              this._instanceController.getStatefulCaptureSet(captureSetID);
            if (captureSet) {
              ...
              if (filterSettings && filterSettings.appendAll) {
                this._instanceController.addCaptureToSet(
                  captureSetID,
                  id,
                  function (result) {
    ...
    appendToUserList: function (id, Capture) {
      if (
        ...
        let watchSubscription = captureStateful.watch(
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          ...
          if (typeof Capture === "object") {
            ...
            let statefulCapture = this.getStatefulCapture(captureID);
            Resolve({
              Success: "Capture Map Created",
              CaptureID: captureID,
              componentKey: newMap._componentKey,
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          if (this.connectControllerCommands === null) {
            Reject({
              Error:
                "connectControllerCommands have not been loaded into Captures Controller",
            });
            return;
          }
          ...
          this.checkAndReturnValidCapture(Capture)
            .then(
              lang.hitch(this, function (Capture) {
                if (Capture && updateRequest === false) {
                  ...
                  this._capturesDatabase
                    ...
                    .catch(
                      lang.hitch(this, function (Error) {
                        console.log(
                          "Controller could not add Capture to Database",
                          Error
                        );
            ...
            .catch(
              lang.hitch(this, function (Error) {
                Reject({
                  Error:
                    "Captures Controller Add function could not validate Capture",
                  info: Error,
                });
              })
            );
          ...
          this._capturesDatabase
            ...
            .then(
              lang.hitch(this, function (Result) {
                ...
                Object.values(this.captures).forEach(
                  lang.hitch(this, function (capture) {
                    if (
                      capture &&
                      capture._id &&
                      capture.capture &&
                      capture.capture.id.toString() === captureID.toString()
                    ) {
    ...
    removeAllCapturesFor: function (userKey) {
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          this._capturesDatabase
            .removeAllCapturesWithUserKey(userKey)
            .then(
              lang.hitch(this, function (CapturesResult) {
                console.log(
                  "All Captures Removed Request Result",
                  CapturesResult
                );
                this._capturesImagesDatabase
                  .removeAllImagesWithUserKey(userKey)
                  .then(
                    lang.hitch(this, function (ImagesResult) {
                      console.log(
                        "All Images Removed Request Result",
                        ImagesResult
                      );

                      Resolve({
                        SUCCESS: "Data removed from bases",
                        Results: {
                          ImagesResult: ImagesResult,
                          CapturesResult: CapturesResult,
                        },
                      });
                    })
                  );
                ...
                for (objectIndex in userCaptures) {
                  if (
                    typeof userCaptures[objectIndex] !== "function" &&
    ...
    updateCaptureImage: function (captureImage) {
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          this.checkAndReturnValidCaptureImage(captureImage)
            .then(
              lang.hitch(this, function (validCaptureImage) {
                if (
                  typeof validCaptureImage === "object" &&
                  typeof validCaptureImage.id === "string" &&
                  typeof validCaptureImage.image === "object" &&
                  typeof validCaptureImage.signature === "object"
                ) {
                  this._capturesImagesDatabase
                    .addCaptureImage(validCaptureImage)
                    .then(
                      lang.hitch(this, function (Result) {
                        ...
                        this.addImageInfoToCaptureStateful(
                          validCaptureImage.signature,
                          this.getCaptureObjectID(validCaptureImage.id)
                        );
                        ...
                        if (
                          validCaptureImage &&
                          validCaptureImage.image &&
                          validCaptureImage.image.data
                        ) {
                          this.ImageUtility.resizeImageBase64(
                            validCaptureImage.image.data,
                            200
                          )
                            .then(
                              lang.hitch(this, function (resizedImage) {
                                validCaptureImage.image = null;
                                validCaptureImage.preview = resizedImage;

                                //save resized image as preview for capture in database
                                this._capturesImagesDatabase
                                  .updateCaptureImagePreview(
                                    resizedImage,
                                    captureImage.id
                                  )
                                  .then(
                    ...
                    .catch(
                      lang.hitch(this, function (Error) {
                        ...
                        Reject({
                          Error: Error,
                          reason:
                            "Controller could not add Capture Image to Database",
                        });
                      })
                    );
                } else {
                  ...
                  Reject({
                    Error:
                      "updateEntry was not as expected in updateCaptureImage",
    ...
    retrieveCaptureImagePreview: function (captureID) {
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          if (captureID) {
            this._capturesImagesDatabase
              .getCaptureImagePreview(captureID)
              .then(
                lang.hitch(this, function (Result) {
                  if (Result) {
                    Resolve(Result);
                    ...
                    this._capturesImagesDatabase
                      .getCaptureImage(captureID)
                      .then(
                        lang.hitch(this, function (CaptureImage) {
                          // console.log("Capture Image😰😰😰", Result)
                          if (
                            CaptureImage &&
                            CaptureImage.image &&
                            CaptureImage.image.data
                          ) {
                            this.ImageUtility.resizeImageBase64(
                              CaptureImage.image.data,
                              200
              ...
              .then(
                lang.hitch(this, function (CaptureImage) {
                  if (CaptureImage && isCaptureImageValid(CaptureImage)) {
                    ...
                    Resolve({
                      computedHash: hash,
                      signatureHash: CaptureImage.signature.Hash,
                    });
                  } else {
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          if (captureID) {
            ...
            this.getCaptureByDeviceID(captureID)
              .then(
                lang.hitch(this, function (Capture) {
                  if (
                    Capture &&
                    Capture.capture &&
                    Capture.capture.signature &&
                    Capture.capture.signature.Hash
                  ) {
                    let hash = this.getCaptureCheckHash(Capture.capture);
                    Resolve({
                      computedHash: hash,
                      signatureHash: Capture.capture.signature.Hash,
                    });
    ...
    getCaptureByDeviceID: function (id) {
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          ...
          this._capturesDatabase.getCaptureByDeviceID(id).then(
            lang.hitch(this, function (Result) {
    ...
    getCapture: function (id) {
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          ...
          this._capturesDatabase.getCapture(id).then(
            lang.hitch(this, function (Result) {
              // console.log("Capture Loaded", Result);
              try {
                Resolve(Result);
    ...
    getCaptureCheckHash: function (Capture) {
      if (
        ...
        let signStringCombination =
          created + barcode + recognizedText + id + note;

        return this.connectControllerCommands.getStringHash(
          signStringCombination
        );
      } else {
        return Capture;
      }
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          if (
            ...
            let device =
              this.connectControllerCommands.getDeviceByPublicSigningKey(
                publicKey
              );

            let checkHash = this.connectControllerCommands.getStringHash(
              signStringCombination
            );
            ...
            if (checkHash === hash) {
              ...
              this.connectControllerCommands
                .verifySignedString(signStringCombination, proof, publicKey)
                .then(
                  lang.hitch(this, function (result) {
                    if (result) {
                      if (
                        device &&
                        device.getOwnerUserKey &&
                        typeof device.getOwnerUserKey == "function"
                      ) {
                      ...
                      Reject({
                        Error: "Signature Could not be verified: Bad signature",
                      });
                    }
                  })
                )
                .catch(
                  lang.hitch(this, function (Error) {
                    ...
                    Reject({
                      Error: "Signature Could not be verified: Error Caught",
                      caughtError: Error,
                    });
                  })
                );
            ...
            Reject({
              Error:
                "Signature Could not be verified: Capture data unexpected format",
            });
          }
        })
      );
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          ...
          if (
            ...
            validCapture.barcode = nodeSanitizeHtml(Capture.barcode);
            validCapture.id = nodeSanitizeHtml(Capture.id);
            validCapture.recognizedText = nodeSanitizeHtml(
            ...
            this.checkCaptureSignature(Capture)
              .then(
                lang.hitch(this, function (validation) {
                  if (validation) {
                    ...
                    Reject({
                      Error:
                        "Signature Could not be verified: validation false",
                    });
                  }
                })
              )
        ...
        lang.hitch(this, function (Resolve, Reject) {
          if (
            ...
            if (checkHash === hash) {
              ...
              console.log(
                "Capture Image Hash Dose Not Match",
                CaptureImage,
                checkHash,
                hash,
                imageString
      ...
      return new Promise(
        lang.hitch(this, function (Resolve, Reject) {
          ...
          if (
            ...
            this.checkCaptureImageSignature(CaptureImage)
              .then(
                lang.hitch(this, function (validation) {
                  if (validation) {
