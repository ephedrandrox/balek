...
], function (declare, lang, Stateful, SyncedMapInterface) {
  return declare("digivigilDigiscanCapturesInterfaceController", null, {
    ...
    loadDetailedImageFor: function (captureID) {
      ...
      if (id && !hasBeenRequested) {
        ...
        this._interface.getCaptureImage(
          id,
          lang.hitch(this, function (imageFetchResult) {
    ...
    isCaptureSyncing: function (captureID) {
      if (this.captureSyncedMaps[captureID]) {
        return true;
      } else {
        return false;
      }
    ...
    getCaptureByID: function (captureID) {
      if (!this.captures[captureID]) {
        ...
        this._interface.getCaptureSyncedMap(
          captureID,
          lang.hitch(this, function (syncedMapKeys) {
            //create a syncedMap Interface and store it by CaptureID
            this.captureSyncedMaps[captureID] = new SyncedMapInterface({
              _instanceKey: syncedMapKeys.instanceKey.toString(),
              _componentKey: syncedMapKeys.componentKey.toString(),
            });
            //set state watcher to update local capture state object
            this.captureSyncedMapWatchHandles[captureID] =
              this.captureSyncedMaps[captureID].setStateWatcher(
                lang.hitch(this, function (name, oldValue, newValue) {
                  ...
                  let imagePreview =
                    this.captures[captureID].get("imagePreview");
                  if (id && imageInfo && !imagePreview) {
                    //load Preview image
                    this._interface.getCaptureImagePreview(
                      id,
                      lang.hitch(this, function (imageFetchResult) {
                        if (imageFetchResult && imageFetchResult.SUCCESS) {
                          let CaptureImage = imageFetchResult.SUCCESS;
