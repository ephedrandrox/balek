  ...
  return declare("balekuteConnectControllerInvitation", null, {
    ...
    useKey: function (invitationKey, deviceInfo) {
      ...
      if (
        ...
        this.statusState.get("status") == "waiting"
      ) {
        ...
        const message = Uint8Array.from(
          Buffer.from(deviceInfo.keychainIdentifier, "utf8")
        );
        const publicKeyBuf = new Buffer(
          publicSigningKey.toString("ascii"),
          "ascii"
        );
        const signatureBuf = new Buffer(
          atob(signature).toString("ascii"),
          "ascii"
        );
    ...
    userAcceptsDevice: function (owner, invitationKey) {
      ...
      if (owner.userKey == this.owner.userKey && invitationKey == this.key) {
        ...
        if (invitationStatus == "used") {
          this._connectController
            .createDevice({
              owner: { userKey: this.owner.userKey },
